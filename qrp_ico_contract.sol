...
contract Token is CappedToken, BurnableToken, Withdrawable {
    ...
    function Token() CappedToken(100000000 ether) StandardToken("Cryptics ERC20 Token", "QRP", 18) public {
...
contract Crowdsale is Manageable, Withdrawable, Pausable {
    ...
    function Crowdsale() public {
        ...
        token.mint(0xBed698c8e0f099fCE780f9870E94024183d9BA52, 3000000 ether);
        token.mint(0x9ce850681bCD535555299fC315f28FB11fB031f7, 11000000 ether);
        token.mint(this, 20000000 ether);
    ...
    function getCommandTokens() onlyOwner public {
        ...
        if(now >= 1546300800 && lcgt < 1546300800) send = 2000000 ether;         // 01.01.2019
        else if(now >= 1554076800 && lcgt < 1554076800) send = 2250000 ether;    // 01.04.2019
        else if(now >= 1561939200 && lcgt < 1561939200) send = 2250000 ether;    // 01.07.2019
        else if(now >= 1569888000 && lcgt < 1569888000) send = 2250000 ether;    // 01.10.2019
        else if(now >= 1577836800 && lcgt < 1577836800) send = 2250000 ether;    // 01.01.2020
        else if(now >= 1585699200 && lcgt < 1585699200) send = 2250000 ether;    // 01.04.2020
        else if(now >= 1593561600 && lcgt < 1593561600) send = 2250000 ether;    // 01.07.2020
        else if(now >= 1601510400 && lcgt < 1601510400) send = 2250000 ether;    // 01.10.2020
        else if(now >= 1609459200 && lcgt < 1609459200) send = 2250000 ether;    // 01.01.2021

        require(send > 0);
        
        lcgt = now;
        token.transfer(0xD45e5AC3fe954A2dfa99865eCf3bB94179a396a3, send);
    }
    
    function withdrawTokens(ERC20 _token, address _to, uint _value) onlyOwner public returns(bool) {
        require(_token != token || lcgt >= 1609459200);
